...
describe('usePreferences', () => {
  ...
  it('isFavourite is case-insensitive', async () => {
    ...
    const { result } = renderHook(() => usePreferences());
    await waitFor(() => expect(result.current.loading).toBe(false));
    expect(result.current.isFavourite({ ...char, name: 'JUMBAA', realmSlug: 'YSONDRE' })).toBe(true);
  });
  ...
  it('toggleFavourite adds a character optimistically', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve({ favourites: [char] }) } as Response)
    );
  ...
  it('toggleFavourite removes a character optimistically', async () => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({ ok: true, json: () => Promise.resolve({ favourites: [] }) } as Response)
    );
  ...
  it('addRecent prepends character and deduplicates', async () => {
    ...
    act(() => { result.current.addRecent(altChar); });
  ...
  it('addRecent caps at 5 entries', async () => {
    const fiveChars = Array.from({ length: 5 }, (_, i) =>
      ({ ...char, name: `Char${i}`, realmSlug: `realm${i}` })
    );
